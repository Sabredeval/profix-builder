document.addEventListener('DOMContentLoaded', function() {
  ...
  supabase
    ...
    .then(async ({ data: projects, error }) => {
      ...
      // For each project, fetch its gallery data if needed for preview
      for (const project of projects) {
        try {
          ...
          if (!galleryError && galleryData && galleryData.gallery) {
            project.gallery = galleryData.gallery;
          }
        } catch (err) {
          console.warn(`Failed to fetch gallery for project ${project.id}:`, err);
          // Continue without gallery data
          project.gallery = [];
        }
      }
      
      // Now render all projects with their gallery data
